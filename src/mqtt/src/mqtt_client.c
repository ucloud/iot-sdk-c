...
void* IOT_MQTT_Construct(MQTTInitParams *pParams)
{
    ...
    if(NULL != pParams->device_secret)
    {
        uiot_rpc_init(mqtt_client,pParams);    
    }
    return mqtt_client;
end:
...
void IOT_MQTT_RPC_Handle(void *pClient, MQTTMessage *message, void *pUserData)
{
    LOG_ERROR("recv:%s\n",message->topic);
    char *response_topic = (char *)HAL_Malloc(MAX_SIZE_OF_CLOUD_TOPIC * sizeof(char));
    if (response_topic == NULL) 
    {
        LOG_ERROR("request_topic malloc fail\n");
        return;
    }    
    memset(response_topic, 0x0, MAX_SIZE_OF_CLOUD_TOPIC);
    strncpy(response_topic,message->topic,strlen(message->topic));
    
    LITE_replace_substr(response_topic, "request", "response");

    LOG_ERROR("response_topic:%s\n",response_topic);
    PublishParams pub_params = DEFAULT_PUB_PARAMS;
    pub_params.qos = QOS1;

    char topic_content[] = "{\"ret\": 0}";

    pub_params.payload = topic_content;
    pub_params.payload_len = strlen(topic_content);

    IOT_MQTT_Publish(pClient, response_topic, &pub_params);
    HAL_Free(response_topic);
    return;
    
}

int uiot_rpc_init(UIoT_Client *pClient, MQTTInitParams *pParams){
    int ret;
    SubscribeParams sub_params = DEFAULT_SUB_PARAMS;
    sub_params.on_message_handler = IOT_MQTT_RPC_Handle;
    char *request_topic = (char *)HAL_Malloc(MAX_SIZE_OF_CLOUD_TOPIC * sizeof(char));
    if (request_topic == NULL) 
    {
        LOG_ERROR("request_topic malloc fail\n");
        return FAILURE_RET;
    }
    memset(request_topic, 0x0, MAX_SIZE_OF_CLOUD_TOPIC);
    HAL_Snprintf(request_topic, MAX_SIZE_OF_CLOUD_TOPIC, RPC_REQUEST_TEMPLATE, pParams->product_sn, pParams->device_sn);

    ret = IOT_MQTT_Subscribe(pClient, request_topic, &sub_params);
    HAL_Free(request_topic);
    return ret;
}
